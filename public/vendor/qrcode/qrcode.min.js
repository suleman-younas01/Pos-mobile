 ...
 *
 * Bundled locally so QR generation works on devices that can't reach the
 * cdnjs CDN (cellular networks, ad-blockers, offline POS). Includes the
 * <img> data-URL emission so receipts can be cloned via innerHTML for
 * print without losing the QR (canvas pixel data does not survive that
 * clone, but the data-URL <img> does).
 */
var QRCode;
...
(function () {
	...
	DrawingCanvas.prototype.makeImage = function () {
		if (this._bIsPainted && this._bSupportDataURI !== false) {
			try {
				this._elImage.src = this._elCanvas.toDataURL("image/png");
				this._elImage.style.display = "block";
			} catch (e) { /* ignore — img stays hidden, canvas remains visible */ }
		}
	};
	DrawingCanvas.prototype.isPainted = function () { return this._bIsPainted; };
	 ...
	 * @param {QRCode.CorrectLevel} [vOption.correctLevel=QRCode.CorrectLevel.H]
	 */
	QRCode = function (el, vOption) {
		this._htOption = {
			width: 256, height: 256, typeNumber: 4,
	...
	QRCode.prototype.makeImage = function () {
		// Original library skipped makeImage() when _getAndroid() returned the
		// boolean `true` (i.e. Android UA where the legacy `Android X.Y` regex
		// did not match — Android 7+ reports just `Android 13`, no decimal).
		// `true >= 3` coerces to `1 >= 3` (false), so on every modern Android
		// the <img> was left with display:none and an empty src, leaving the
		// receipt QR blank. The legacy guard targeted Android < 3 (data-URI
		// support); all supported devices today handle data URIs fine, so we
		// always emit the <img>.
	...
	// Marker so callers can verify they got the full library (not the
	// previously-shipped broken minimal build that produced blank canvases).
	QRCode.__fullLib = true;
