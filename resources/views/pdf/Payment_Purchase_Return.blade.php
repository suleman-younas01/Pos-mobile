...
<body class="{{ $isRtl ? 'rtl' : '' }}" dir="{{ $isRtl ? 'rtl' : 'ltr' }}">
                ...
                <div style="border: 1px solid #e5e7eb; border-radius: 4px; overflow: hidden;">
                    ...
                    <div style="padding: 10px; background: #fffbeb;">
                        ...
                        <div style="font-size: 7.5pt; color: #6b7280; line-height: 1.6;">
                            <div><strong style="color: #1f2937;">{{ __('pdf.phone') }}{{ $isRtl ? '' : ':' }}</strong> {{$payment['supplier_phone']}}</div>
                            <div><strong style="color: #1f2937;">{{ __('pdf.email') }}{{ $isRtl ? '' : ':' }}</strong> {{$payment['supplier_email']}}</div>
                            <div><strong style="color: #1f2937;">{{ __('pdf.address') }}{{ $isRtl ? '' : ':' }}</strong> {{$payment['supplier_adr']}}</div>
                        </div>
                    </div>
                </div>
            </td>
    ...
    <table style="width: 100%; border-collapse: collapse; margin-bottom: 15px; border: 1px solid #e5e7eb;" cellpadding="0" cellspacing="0">
        <thead>
            <tr style="background: #f59e0b;">
                ...
                <th style="padding: 10px; text-align: right; font-size: 9pt; font-weight: bold; color: #ffffff; text-transform: uppercase;">{{ __('pdf.amount_received') }}</th>
            </tr>
        </thead>
        <tbody>
    ...
    <div style="margin-top: 30px; padding-top: 10px; border-top: 2px solid #e5e7eb;">
        ...
        <div style="text-align: center; padding: 8px 0;">
            <p style="font-size: 10pt; font-weight: bold; color: #f59e0b; margin: 0; letter-spacing: 0.3px;">{{ __('pdf.payment_received_successfully') }}</p>
        </div>
    </div>
</body>
</html>
